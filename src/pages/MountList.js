...
{
    // checks whether mount is already part of collection
  ...
  // checks whether mount is already on wishlist
  ...
  const loaded = () => {
    if (uniqueMount.length === 0) { // when the search result doesn't return any mount(s)
      return (
        <div>
          <Card border="dark">
            <Card.Body>
            ...
            <Card.Img
              style={{ width: "300px" }}
              src="https://i.pinimg.com/originals/08/c8/83/08c883567cb023ade97c70d841d1b3b5.png"
            />
          </Card>
        </div>
      );
      ...
      return (
        <div>
          <button onClick={() => sortList(uniqueMount, order)}>
            Sort List
          </button>
